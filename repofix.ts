...
const COMMUNITY_DB_URL = 'https://raw.githubusercontent.com/your-org/monocheck/main/special-cases.json';
...
async function fetchCommunitySolutions(): Promise<CommunitySolution[]> {
  ...
  if (existsSync(cachePath)) {
    ...
    if (Date.now() - stats.mtimeMs < ttl) {
      console.log(chalk.gray('Using cached community solutions'));
  ...
  try {
    ...
    console.log(chalk.green('Fetched and cached community solutions'));
    return solutions;
  } catch (err) {
    console.log(chalk.yellow(`Warning: Failed to fetch community solutions (${err.message}).`));
    const localPath = join(__dirname, 'special-cases.json');
    if (existsSync(localPath)) {
      console.log(chalk.gray('Using local special-cases.json'));
      return JSON.parse(readFileSync(localPath, 'utf-8'));
    }
    console.log(chalk.yellow('No local fallback found. Continuing without community solutions.'));
    return [];
  }
}
...
async function initializeConfig() {
  console.log(chalk.blue('Welcome to monocheck configuration setup!'));
  ...
  if (mode === '1' || mode === '2') {
    if (mode === '1') {
      ...
      for (const dir of config.directories) {
        if (!existsSync(dir.path)) {
          ...
          console.log(chalk.yellow(`No 'src' dir found for ${dir.tsconfig}. Using ${dir.path} instead.`));
        }
    ...
    while (readlineSync.keyInYN(chalk.green('Add a special case?'))) {
      const category = readlineSync.question(chalk.white('Enter category (e.g., nextjs, expo) or "all": '));
...
function createTuiDashboard(issuesByDir: { [dir: string]: ImportIssue[] }, onFix: (issue: ImportIssue, dir: string) => Promise<void>) {
  const screen = blessed.screen({
    smartCSR: true,
    title: 'monocheck Dashboard',
  });
  ...
  const table = blessed.listtable({
    ...
    height: '80%',
    border: { type: 'line' },
    style: { border: { fg: 'cyan' }, header: { fg: 'green', bold: true }, cell: { fg: 'white' } },
    keys: true,
    tags: true,
    noCellBorders: false,
  });
  ...
  const status = blessed.text({
    ...
    width: '100%',
    height: '20%',
    content: 'Use arrow keys to navigate, Enter to fix, q to quit',
    style: { fg: 'yellow' },
  });

  const allIssues: Array<{ dir: string; issue: ImportIssue }> = [];
  Object.entries(issuesByDir).forEach(([dir, issues]) => {
    issues.forEach((issue) => allIssues.push({ dir, issue }));
  });

  const updateTable = () => {
    const data = [['File', 'Line', 'Import', 'Issue', 'Suggestion', 'Status']];
    allIssues.forEach(({ issue }) => {
      data.push([
        ...
        issue.fixed ? 'Fixed' : issue.userChoice || 'Pending',
      ]);
    });
    table.setData(data);
  };
  ...
  table.on('select', async (item, index) => {
    ...
    const { dir, issue } = allIssues[index - 1];
    if (!issue.fixed && issue.suggestion) {
      status.setContent(`Fixing: ${issue.file}:${issue.line}...`);
      screen.render();
      await onFix(issue, dir);
      updateTable();
      status.setContent('Fix applied. Use arrow keys to navigate, Enter to fix, q to quit');
      screen.render();
    }
  });
...
async function monocheck() {
  ...
  const USE_TUI = process.argv.includes('--tui');

  const communitySolutions = await fetchCommunitySolutions();
  const issuesByDir: { [dir: string]: ImportIssue[] } = {};

  for (const dir of config.directories) {
    console.log(chalk.blue(`\nStarting monocheck for ${dir.path}`));
    console.log(chalk.gray(`Auto-fix: ${AUTO_FIX}${INTERACTIVE ? ' (interactive)' : ''}, Dry-run: ${DRY_RUN}, TUI: ${USE_TUI}`));
    ...
    console.log(chalk.blue(`\nCompleted monocheck for ${files.length} files in ${dir.path}`));
    console.log(
      chalk.cyan(
  ...
  if (USE_TUI && AUTO_FIX && !DRY_RUN) {
    createTuiDashboard(issuesByDir, async (issue, dirPath) => {
      const dir = config.directories.find((d) => d.path === dirPath)!;
      const project = new Project({ tsConfigFilePath: dir.tsconfig });
      ...
      if (importDecl && issue.suggestion?.includes('Change to: import')) {
      ...
      } else if (issue.commented && issue.suggestion?.includes('Uncomment and change to')) {
        const newImportPath = issue.suggestion.match(/from '([^']+)'/)?.[1];
        if (newImportPath) {
          const fullText = file.getFullText();
          const lines = fullText.split('\n');
          lines[issue.line - 1] = lines[issue.line - 1]
            .replace(/^\s*\/\/+\s*|^#+\s*|\/\*|\*\//g, '')
            .replace(issue.importPath, newImportPath);
          file.replaceWithText(lines.join('\n'));
          issue.fixed = true;
          await file.save();
        }
...
monocheck().catch((err) => {
  console.error(chalk.red('Error running monocheck:', err));
  process.exit(1);
});